...
fn now() -> chrono::DateTime<Local> {
    Local.with_ymd_and_hms(2026, 5, 8, 14, 0, 0).unwrap()
}

fn discharging(p: u8) -> BatteryReadingSnapshot {
    BatteryReadingSnapshot {
        percent: p,
...
fn critical_re_fires_after_period() {
    let s = State {
        last_critical_notified_at: Some(now() - chrono::Duration::minutes(35)),
        ..State::default()
    };
...
fn critical_silent_within_period() {
    let s = State {
        last_critical_notified_at: Some(now() - chrono::Duration::minutes(10)),
        ..State::default()
    };
...
fn charging_silences_everything() {
    ...
    let r = decide(
        &BatteryReadingSnapshot {
            percent: 4,
            charging: ChargingState::Recharging,
        },
        &s,
        &cfg(),
        now(),
...
fn mute_until_silences_everything() {
    let s = State {
        mute_until: Some(now() + chrono::Duration::hours(1)),
        ..State::default()
    };
    let r = decide(&discharging(4), &s, &cfg(), now());
    assert!(r.is_none());
...
fn warn_disabled_skips_warn_but_critical_still_fires() {
    ...
    assert_eq!(
        decide(&discharging(4), &s, &c, now()).unwrap(),
...
fn warn_re_arms_after_rise_above_threshold_plus_hysteresis() {
    ...
    let mut s = State {
        last_warn_notified_date: Some(now().date_naive()),
        ..State::default()
    };
    clear_armed_if_rose(
        &BatteryReadingSnapshot {
    ...
    assert_eq!(
        s.last_warn_notified_date, None,
        "rising above warn+hysteresis re-arms warn"
    );
}

#[test]
fn warn_silenced_when_critical_rate_limited() {
    // Critical fired 10 min ago; we're still <= critical (4%).
    // Warn period (24h) has elapsed since None, but critical period (30m) hasn't.
    let s = State {
        last_critical_notified_at: Some(now() - chrono::Duration::minutes(10)),
        ..State::default()
    };
    ...
    assert!(
        r.is_none(),
        "warn must not fire while critical is rate-limited"
    );
}
